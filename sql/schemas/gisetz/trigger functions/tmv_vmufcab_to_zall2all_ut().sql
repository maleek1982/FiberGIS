...
--
-- Name: tmv_vmufcab_to_zall2all_ut(); Type: FUNCTION; Schema: gisetz; Owner: -
--

CREATE FUNCTION tmv_vmufcab_to_zall2all_ut() RETURNS trigger
...
BEGIN
    ...
    -- GO
    IF ((NEW.descr <> OLD.descr) and ( NEW.descr ~* '[[:digit:]+]%$')) THEN
        
        DELETE FROM z_all2all WHERE p_table_uuid = NEW.uu_id;
    ...
    -- GO
    IF (
            ((NEW.descr <> OLD.descr) and ( NEW.descr IN ('1:n','1:2','1:4','1:8')) and (NEW.vol_use > 0 )) or
            ((NEW.vol_use <> OLD.vol_use) and ( NEW.descr IN ('1:n','1:2','1:4','1:8')) and (NEW.vol_use > 0 ))
        ) THEN
...
--
-- Name: tmv_vmufcab_to_zall2all_ut(); Type: ACL; Schema: gisetz; Owner: -
--

REVOKE ALL ON FUNCTION tmv_vmufcab_to_zall2all_ut() FROM PUBLIC;
REVOKE ALL ON FUNCTION tmv_vmufcab_to_zall2all_ut() FROM postgres;
GRANT ALL ON FUNCTION tmv_vmufcab_to_zall2all_ut() TO postgres;
GRANT ALL ON FUNCTION tmv_vmufcab_to_zall2all_ut() TO PUBLIC;
GRANT ALL ON FUNCTION tmv_vmufcab_to_zall2all_ut() TO read_roles WITH GRANT OPTION;
